...
public class BookFlexDriveController implements Initializable, FlexTrafikController, Observer {

	private FTController ftController;
	private Stage stage;
	private FlexDrive flexDrive;
	...
	public void btnOrderTrip(ActionEvent event) {
		if (!errorsList.hasErrors() && validateMandatoryFields()) {
			...
			flexDrive.setUserId(user.getUserId());
			...
			ftController.bookFlexDrive(flexDrive);
		}
	}
	...
	public void init() {
		...
		btnOrderTrip.setDisable(true);

		labelPrice.textProperty().addListener(new ChangeListener<String>() {
			@Override
			public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
				if (!newValue.equalsIgnoreCase("price") && !newValue.isEmpty()) {
					progressIndicator.setVisible(false);
					btnOrderTrip.setDisable(false);
				} else {
	...
	public void update(Observable observable, Object state) {
		if (state == State.FLEXDRIVE_BOOKED) {
			flexDrive = null;
			new AlertBox("Din bestilling blev bestilt korrekt", AlertType.INFORMATION);
		}
